...
def get_dam_discharge_from_file():
    ...
    try:
        ...
        print("⚠️ ไม่พบไฟล์ dam_data.txt! จะใช้ค่า default = 1000 ลบ.ม./วินาที")
        return 1000
    except Exception as e:
        print(f"เกิดข้อผิดพลาดในการอ่านไฟล์ dam_data.txt: {e}")
        return 1000

# --- ส่วนที่ 2: วิเคราะห์และสร้างข้อความ ---
def analyze_and_create_message(inburi_level, dam_discharge):
    ...
    if dam_discharge > 2400 or distance_to_bank < 1.0:
        ...
        recommendation = (
            "สรุป: สถานการณ์น้ำยังปกติ ไม่น่ากังวล ขอให้ประชาชนใช้ชีวิตได้ตามปกติครับ"
        )
...
# --- ส่วนที่ 3: ส่งข้อความไปที่ LINE ---
def send_line_broadcast(message):
    if not LINE_TOKEN:
        print("❌ ไม่พบ LINE_CHANNEL_ACCESS_TOKEN! กรุณาตั้งค่าใน GitHub Secrets")
        return
